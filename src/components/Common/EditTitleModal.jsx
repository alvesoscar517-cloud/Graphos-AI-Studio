...
const EditTitleModal = ({ isOpen, currentTitle, onSave, onClose, maxLength = 100 }) => {
  ...
  return (
    <div 
      className={cn("fixed inset-0 flex items-center justify-center z-menu","bg-black/5 backdrop-blur-[1px]","max-md:items-end max-md:p-3"
      )}
      onClick={handleOverlayClick}
    >
      <div className={cn("bg-bg-primary border border-border-light rounded-2xl shadow-popup","w-full max-w-md m-4 overflow-hidden p-5","animate-slide-up-fast","max-md:max-w-full max-md:m-0 max-md:rounded-t-2xl max-md:rounded-b-none"
      )}>
        {/* Header */}
