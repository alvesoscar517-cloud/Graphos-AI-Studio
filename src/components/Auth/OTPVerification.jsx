...
const OTPVerification = ({ email, onVerify, onResend, onCancel, isLoading }) => {
  ...
  return (
    <div className="text-center py-1.5 sm:py-2.5">
      <div className="mb-5 sm:mb-7">
        ...
        <h3 className="m-0 mb-2 sm:mb-2.5 text-lg sm:text-xl font-bold text-gray-900">{t('auth.email.verifyTitle')}</h3>
        <p className="m-0 text-xs sm:text-sm text-gray-600 leading-relaxed">{t('auth.email.verifySubtitle', { email: '' })}<br/><strong className="text-gray-900 font-semibold block mt-1 text-sm sm:text-md break-all">{email}</strong></p>
      </div>
      
      {/* OTP Inputs */}
      <div className="flex justify-center gap-1.5 sm:gap-2.5 mb-4 sm:mb-5">
        {otp.map((digit, index) => (
      ...
      <div className="mb-5 sm:mb-6 text-xs sm:text-sm text-gray-600 font-medium">
        {countdown > 0 ? (
      ...
      <div className="flex flex-col gap-2.5 sm:gap-3">
        ...
        <button type="button" onClick={handleResend} disabled={isLoading || resendCooldown > 0}
          className="bg-transparent border-none text-system-blue text-xs sm:text-sm font-medium px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg hover:bg-system-blue/10 active:bg-system-blue/15 disabled:opacity-40 disabled:cursor-not-allowed transition-all duration-200">
          {resendCooldown > 0 ? t('auth.email.resendIn', { seconds: resendCooldown }) : t('auth.email.resendCode')}
        </button>
        <button type="button" onClick={onCancel} disabled={isLoading}
          className="text-gray-500 text-xs sm:text-sm font-medium mt-1 sm:mt-2 hover:text-gray-700 active:text-gray-800 disabled:opacity-40 disabled:cursor-not-allowed transition-all duration-200">
          {t('auth.email.cancel')}
        </button>
      </div>
    </div>
