...
const DeviationCard = ({ disabled, currentProfile, text, onAnalysisComplete, onAnalysisStart, onAnalysisEnd }) => {
  ...
  const findDeviations = async () => {
    ...
    try {
      ...
      modal.errorWithReport(localizedError || t('analysis.analysisFailed'), error, 'Error', 'DeviationCard.findDeviations')
    } finally {
      setIsLoading(false)
      stopProcessing()
    ...
    <div className={cn("p-3 border border-border-light rounded-xl","bg-bg-secondary transition-all duration-200 hover:shadow-md"
      ...
      <div className="flex items-center gap-3 mb-2 relative">
        ...
        <div className="flex-1 min-w-0">
          <h4 className="text-xs font-medium text-text-primary m-0 mb-0.5">{t('analysis.deviations')}</h4>
          <p className="text-[11px] text-text-secondary m-0">{t('analysis.suggestionsHighlights')}</p>
        </div>
        {analysisData && (
          <button 
