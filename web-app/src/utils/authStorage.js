...
import { isChromeExtension as checkIsChromeExtension, isChromeApiAvailable } from './environment'
 ...
 * - Uses localStorage/sessionStorage for web app based on Remember Me preference
 * 
...
/**
 * Check if running in Chrome Extension context with storage API available
 * Uses the centralized environment detection module
 */
function isChromeExtension() {
  return checkIsChromeExtension() && isChromeApiAvailable('storage.local')
}
...
/**
 * Get the appropriate web storage based on Remember Me preference
 * - localStorage: persistent storage (Remember Me enabled)
 * - sessionStorage: session-only storage (Remember Me disabled)
 * 
 * Requirements: 3.3, 3.4
 */
function getStorage() {
  // Check rememberMe from localStorage first (it's always stored there)
 ...
 * For web app: checks both localStorage and sessionStorage for compatibility
 */
function getFromStorage(key) {
  ...
  // For web app: check primary storage based on Remember Me
 ...
 * For web app: uses localStorage or sessionStorage based on Remember Me
 * 
 * Requirements: 3.3, 3.4
 */
function setToStorage(key, value) {
  ...
  // For web app: use appropriate storage based on Remember Me
  // Note: 'rememberMe' key itself is always stored in localStorage
  if (key === 'rememberMe') {
 ...
 * For web app: removes from both storages to ensure cleanup
 */
function removeFromStorage(key) {
  ...
  // For web app: remove from both storages
...
// Export getStorage for testing purposes
export { getStorage }
