/**
 * Deviations - SEO-optimized Deviations feature page
...
const DeviationBackground = () => (
  <div className="absolute inset-0 overflow-hidden -z-10">
    ...
    {/* SVG Deviation Visualization - Responsive sizing */}
    <svg 
    ...
    {/* Ambient glow - orange - Responsive */}
    <div 
  ...
  return (
    ...
    >
      <div className="relative px-3 py-2.5 sm:px-4 sm:py-3 md:px-6 md:py-4 bg-white/60 dark:bg-white/5 backdrop-blur-sm rounded-xl sm:rounded-2xl border border-white/50 dark:border-white/10 shadow-lg shadow-black/5">
        ...
        <div className="absolute inset-0 rounded-xl sm:rounded-2xl bg-gradient-to-br from-orange-500/20 to-amber-500/20 opacity-0 group-hover:opacity-100 transition-opacity duration-300 -z-10 blur-xl" />
        
        <div className="text-center">
          <div className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-bold bg-gradient-to-r from-orange-600 via-amber-600 to-orange-600 bg-clip-text text-transparent">
            {displayValue}
          </div>
...
const SeverityExplanationSection = ({ t }) => {
  ...
  return (
    <section className="py-12 sm:py-16 md:py-20 lg:py-28 relative overflow-hidden bg-bg-secondary">
      ...
      <div className="absolute top-1/4 right-0 w-48 sm:w-64 md:w-80 lg:w-96 h-48 sm:h-64 md:h-80 lg:h-96 bg-orange-500/5 rounded-full blur-3xl" />
      <div className="absolute bottom-1/4 left-0 w-40 sm:w-56 md:w-72 lg:w-80 h-40 sm:h-56 md:h-72 lg:h-80 bg-amber-500/5 rounded-full blur-3xl" />
      ...
      <div className="w-full max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 xl:px-12 2xl:px-16 relative">
        <div className="text-center mb-8 sm:mb-10 md:mb-12">
          <motion.span 
            ...
            viewport={{ once: true }} 
            className="inline-flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-1.5 sm:py-2 bg-gradient-to-r from-orange-500 to-amber-500 text-white text-xs sm:text-sm font-semibold rounded-full mb-3 sm:mb-4 shadow-lg shadow-orange-500/20"
          >
        ...
        <div className="bg-white/50 dark:bg-slate-800/50 backdrop-blur-sm rounded-2xl sm:rounded-3xl border border-gray-100 dark:border-slate-700/50 p-4 sm:p-6 md:p-8">
          <div className="grid lg:grid-cols-2 gap-6 sm:gap-8 items-center">
            {/* Visual Display - Responsive */}
            <div className="relative">
              <div className="relative w-36 h-36 sm:w-40 sm:h-40 md:w-48 md:h-48 mx-auto mb-4 sm:mb-6">
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <motion.div
                    ...
                    className={`w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 rounded-xl sm:rounded-2xl ${colorClasses[currentSeverity.color].light} flex items-center justify-center`}
                  >
                    <Icon name={currentSeverity.icon} size="lg" className={`icon-${currentSeverity.color} sm:hidden`} />
                    <Icon name={currentSeverity.icon} size="xl" className={`icon-${currentSeverity.color} hidden sm:block`} />
                  </motion.div>
                </div>
              </div>
              
              {/* Current Severity Label - Responsive */}
              <motion.div 
              ...
              >
                <div className={`inline-flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-1.5 sm:py-2 rounded-full ${colorClasses[currentSeverity.color].light}`}>
                  <Icon name={currentSeverity.icon} size="sm" className={`icon-${currentSeverity.color}`} />
                  <span className={`text-sm sm:text-base font-semibold ${colorClasses[currentSeverity.color].text}`}>{currentSeverity.label}</span>
                </div>
                <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 mt-2 sm:mt-3 px-2">{currentSeverity.desc}</p>
                <p className="text-xs text-gray-400 dark:text-gray-500 mt-1.5 sm:mt-2 italic px-2">"{currentSeverity.example}"</p>
              </motion.div>
            </div>
          
            {/* Severity List - Responsive */}
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3 sm:gap-4">
                      <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-lg sm:rounded-xl ${colorClasses[severity.color].light} flex items-center justify-center flex-shrink-0`}>
                        <Icon name={severity.icon} size="md" className={`icon-${severity.color} sm:hidden`} />
                        <Icon name={severity.icon} size="lg" className={`icon-${severity.color} hidden sm:block`} />
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between gap-2">
                          <span className={`text-sm sm:text-base font-semibold ${isActive ? colorClasses[severity.color].text : 'text-gray-800 dark:text-gray-200'}`}>{severity.label}</span>
                          <span className={`text-xs sm:text-sm font-medium ${colorClasses[severity.color].text} flex-shrink-0`}>{severity.threshold}</span>
                        </div>
                        <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 mt-0.5 line-clamp-2">{severity.desc}</p>
                      </div>
                    </div>
                  </motion.button>
                )
...
const HowItWorksSection = ({ t, steps }) => {
  return (
    <section className="py-12 sm:py-16 md:py-20 lg:py-28 relative overflow-hidden">
      {/* Background decoration - Responsive */}
      <div className="absolute top-1/4 left-0 w-48 sm:w-64 md:w-80 lg:w-96 h-48 sm:h-64 md:h-80 lg:h-96 bg-orange-500/5 rounded-full blur-3xl" />
      <div className="absolute bottom-1/4 right-0 w-40 sm:w-56 md:w-72 lg:w-80 h-40 sm:h-56 md:h-72 lg:h-80 bg-amber-500/5 rounded-full blur-3xl" />
          ...
          {steps.map((step, index) => (
            ...
            >
              ...
              <div className="relative bg-white dark:bg-slate-800 rounded-xl sm:rounded-2xl p-4 sm:p-5 md:p-6 border border-gray-200 dark:border-slate-700 shadow-lg hover:shadow-xl transition-shadow h-full">
                ...
                <div className="absolute -top-2 -left-2 sm:-top-3 sm:-left-3 w-6 h-6 sm:w-8 sm:h-8 bg-gradient-to-br from-orange-500 to-amber-500 rounded-full flex items-center justify-center text-white text-xs sm:text-sm font-bold shadow-lg">
                  {step.step}
                </div>
                
                <div className="w-10 h-10 sm:w-12 sm:h-12 md:w-14 md:h-14 bg-orange-100 dark:bg-orange-500/20 rounded-xl sm:rounded-2xl flex items-center justify-center mb-3 sm:mb-4">
                  <Icon name={step.icon} size="lg" className="icon-orange sm:hidden" />
                  <Icon name={step.icon} size="xl" className="icon-orange hidden sm:block" />
                </div>
                
                <h3 className="text-sm sm:text-base md:text-lg font-semibold text-gray-800 dark:text-gray-200 mb-1 sm:mb-2">{step.title}</h3>
                <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 line-clamp-3">{step.desc}</p>
              </div>
            </motion.div>
          ))}
        </div>
      ...
      <div className="w-full max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 xl:px-12 2xl:px-16">
          ...
          {useCases.map((useCase, index) => (
            ...
            >
              <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-lg sm:rounded-xl mb-3 sm:mb-4 flex items-center justify-center ${
                ...
                <Icon name={useCase.icon} size="md" className={`${selectedCase === index ? 'icon-white' : 'icon-orange'} sm:hidden`} />
                <Icon name={useCase.icon} size="lg" className={`${selectedCase === index ? 'icon-white' : 'icon-orange'} hidden sm:block`} />
              </div>
        ...
        <AnimatePresence mode="wait">
          <motion.div
            ...
            exit={{ opacity: 0, y: -20 }}
            className="bg-gradient-to-br from-orange-50 via-amber-50/50 to-white dark:from-slate-800 dark:via-orange-900/20 dark:to-slate-900 rounded-2xl sm:rounded-3xl border border-orange-200/50 dark:border-orange-500/20 p-4 sm:p-6 md:p-8"
          >
            <div className="grid lg:grid-cols-2 gap-4 sm:gap-6 md:gap-8 items-center">
              <div>
                <div className="flex items-center gap-3 sm:gap-4 mb-3 sm:mb-4">
                  <div className="w-12 h-12 sm:w-14 sm:h-14 bg-gradient-to-br from-orange-500 to-amber-500 rounded-xl sm:rounded-2xl flex items-center justify-center shadow-lg shadow-orange-500/30 flex-shrink-0">
                  ...
                  <div>
                    ...
                    <p className="text-xs sm:text-sm text-orange-600 dark:text-orange-400">{t('features.deviations.useCases.spotlight', 'Featured Use Case')}</p>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-1.5 sm:gap-2">
                  {useCases[selectedCase].benefits?.map((benefit, i) => (
                    <span key={i} className="inline-flex items-center gap-1 sm:gap-1.5 px-2 sm:px-3 py-1 sm:py-1.5 bg-orange-100 dark:bg-orange-500/20 rounded-full text-xs sm:text-sm text-orange-700 dark:text-orange-300 font-medium">
              ...
              <div className="bg-white dark:bg-slate-800 rounded-lg sm:rounded-xl p-4 sm:p-5 md:p-6 border border-gray-200 dark:border-slate-700 shadow-lg">
                <div className="flex items-center gap-1.5 sm:gap-2 mb-3 sm:mb-4">
                  <Icon name="quote" size="sm" className="icon-orange" />
                  <span className="text-xs sm:text-sm font-medium text-gray-500 dark:text-gray-400">{t('features.deviations.useCases.example', 'Example')}</span>
                </div>
...
const BenefitsSection = ({ t, benefits }) => {
  ...
  return (
    <section className="py-12 sm:py-16 md:py-20 lg:py-28 relative overflow-hidden">
      {/* Background decoration - Responsive */}
      <div className="absolute top-1/3 left-0 w-48 sm:w-56 md:w-64 lg:w-72 h-48 sm:h-56 md:h-64 lg:h-72 bg-orange-500/5 rounded-full blur-3xl" />
      <div className="absolute bottom-1/3 right-0 w-56 sm:w-72 md:w-80 lg:w-96 h-56 sm:h-72 md:h-80 lg:h-96 bg-amber-500/5 rounded-full blur-3xl" />
        ...
        <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 md:gap-5">
          {enhancedBenefits.map((benefit, index) => (
            <motion.div
              ...
              className="group relative bg-white dark:bg-slate-800 rounded-xl sm:rounded-2xl p-4 sm:p-5 md:p-6 border border-gray-200 dark:border-slate-700 shadow-lg hover:shadow-2xl hover:border-orange-200 dark:hover:border-orange-500/30 transition-all duration-300"
            >
              <div className="absolute inset-0 rounded-xl sm:rounded-2xl bg-gradient-to-br from-orange-500/10 to-amber-500/10 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
              
              <div className="absolute -top-1.5 -right-1.5 sm:-top-2 sm:-right-2 w-5 h-5 sm:w-6 sm:h-6 md:w-7 md:h-7 bg-gradient-to-br from-orange-500 to-amber-500 rounded-full flex items-center justify-center text-white text-[10px] sm:text-xs font-bold shadow-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                {index + 1}
              </div>
              
              <div className="relative">
                <div className="w-10 h-10 sm:w-12 sm:h-12 md:w-14 md:h-14 bg-gradient-to-br from-orange-500 to-amber-500 rounded-xl sm:rounded-2xl flex items-center justify-center mb-3 sm:mb-4 shadow-lg shadow-orange-500/20 group-hover:shadow-orange-500/40 transition-shadow">
                ...
                <div className="flex items-baseline gap-1 mb-2 sm:mb-3">
                  <span className="text-lg sm:text-xl md:text-2xl font-bold bg-gradient-to-r from-orange-600 to-amber-600 bg-clip-text text-transparent">
                    {benefit.stat}
                  </span>
  ...
  return (
    <section className="py-12 sm:py-16 md:py-20 lg:py-28 relative overflow-hidden bg-bg-secondary">
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-0 right-0 w-48 sm:w-64 md:w-80 lg:w-96 h-48 sm:h-64 md:h-80 lg:h-96 bg-orange-500/5 rounded-full blur-3xl" />
        <div className="absolute bottom-0 left-0 w-40 sm:w-56 md:w-72 lg:w-80 h-40 sm:h-56 md:h-72 lg:h-80 bg-amber-500/5 rounded-full blur-3xl" />
      </div>
      ...
      <div className="w-full max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        ...
        <div className="text-center mb-8 sm:mb-10 md:mb-12">
          <motion.span 
            ...
            whileInView={{ opacity: 1, scale: 1 }} 
            viewport={{ once: true }} 
            className="inline-flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-1.5 sm:py-2 bg-bg-primary text-orange-600 text-xs sm:text-sm font-semibold rounded-full mb-3 sm:mb-4 border border-gray-200 dark:border-gray-700 shadow-sm"
          >
        ...
        {/* FAQ Items - Using optimized CSS grid animation */}
        <FAQAccordion 
          faqs={faqs} 
          openFaq={openFaq} 
        ...
        {/* Contact Support Card - Responsive */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
        ...
        >
          <div className="w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 mx-auto mb-3 sm:mb-4 bg-orange-500/10 rounded-lg sm:rounded-xl flex items-center justify-center">
            <Icon name="message-circle" size="lg" className="icon-orange sm:hidden" />
            <Icon name="message-circle" size="xl" className="icon-orange hidden sm:block" />
          </div>
          ...
          <p className="text-sm sm:text-base text-text-secondary mb-4 sm:mb-5 md:mb-6 max-w-md mx-auto px-2">
          ...
          <motion.a
            ...
            whileTap={{ scale: 0.98 }}
            className="inline-flex items-center gap-1.5 sm:gap-2 px-4 sm:px-5 md:px-6 py-2.5 sm:py-3 bg-gradient-to-r from-orange-500 to-amber-500 text-white rounded-lg sm:rounded-xl font-semibold text-sm sm:text-base hover:from-orange-600 hover:to-amber-600 transition-all shadow-sm hover:shadow-md"
          >
    ...
    <section className="pt-0 pb-12 sm:pb-16 md:pb-20 lg:pb-28 bg-bg-secondary relative overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <motion.div 
          ...
          className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[400px] sm:w-[600px] md:w-[750px] lg:w-[900px] h-[400px] sm:h-[600px] md:h-[750px] lg:h-[900px] bg-orange-500/5 rounded-full blur-3xl" 
        />
      </div>
          ...
          <div className="relative p-6 sm:p-8 md:p-10 lg:p-14 xl:p-20 text-center">
            ...
            >
              <motion.a
                ...
                whileHover={{ scale: 1.02, y: -1 }}
                whileTap={{ scale: 0.98 }}
                className="group inline-flex items-center justify-center gap-2 px-5 sm:px-6 md:px-8 py-3 sm:py-3.5 md:py-4 bg-white text-orange-600 rounded-lg sm:rounded-xl font-bold text-sm sm:text-base md:text-lg shadow-sm hover:shadow-lg transition-all"
              >
                <span>{t('cta.getStartedFree', 'Get Started Free')}</span>
                <Icon name="arrow-right" size="sm" className="icon-orange group-hover:translate-x-0.5 transition-transform sm:hidden" />
                <Icon name="arrow-right" size="md" className="icon-orange group-hover:translate-x-0.5 transition-transform hidden sm:block" />
              </motion.a>
              <motion.a
                href="#demo"
                whileHover={{ scale: 1.02, y: -1 }}
      ...
      <div className="relative">
        ...
        <section className="relative min-h-[85vh] sm:min-h-[88vh] lg:min-h-[92vh] flex items-center justify-center overflow-hidden pt-16 sm:pt-12 md:pt-8 pb-12 sm:pb-14 md:pb-16">
          ...
          <div className="w-full max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 xl:px-12 2xl:px-16 text-center relative z-10">
            ...
            >
              <motion.span 
                className="relative inline-flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 md:px-5 py-2 sm:py-2.5 bg-gradient-to-r from-orange-500/10 via-orange-500/15 to-amber-500/10 text-orange-600 dark:text-orange-400 text-xs sm:text-sm font-semibold rounded-full border border-orange-500/20 shadow-lg shadow-orange-500/10"
                animate={{
                  boxShadow: [
            ...
            <motion.div
              ...
              className="grid grid-cols-3 gap-2 sm:gap-3 md:gap-4 mb-6 sm:mb-8 md:mb-10 max-w-xs sm:max-w-md md:max-w-xl lg:max-w-2xl mx-auto"
            >
            ...
            >
              <motion.a
                ...
                whileTap={{ scale: 0.98 }}
                className="group relative inline-flex items-center justify-center gap-2 px-5 sm:px-6 md:px-8 py-3 sm:py-3.5 md:py-4 bg-gradient-to-r from-orange-500 to-amber-500 text-white rounded-xl font-semibold text-sm sm:text-base md:text-lg overflow-hidden shadow-xl shadow-orange-500/30"
              >
                {/* Shine effect */}
              ...
              <motion.a
                ...
                whileHover={{ scale: 1.02, y: -2 }}
                whileTap={{ scale: 0.98 }}
                className="inline-flex items-center justify-center gap-2 px-5 sm:px-6 md:px-8 py-3 sm:py-3.5 md:py-4 bg-white/80 dark:bg-white/10 backdrop-blur-sm border border-orange-200 dark:border-orange-500/30 text-orange-600 dark:text-orange-400 rounded-xl font-semibold text-sm sm:text-base md:text-lg hover:bg-white dark:hover:bg-white/20 transition-all shadow-lg"
              >
                <Icon name="play-circle" size="sm" className="icon-orange" />
                {t('cta.tryDemo', 'Try Demo')}
              </motion.a>
            </motion.div>
            ...
            >
              {[
                { icon: 'credit-card', text: t('features.deviations.trust.noCard', 'No credit card required') },
                { icon: 'zap', text: t('features.deviations.trust.instant', 'Instant results') },
                { icon: 'shield', text: t('features.deviations.trust.privacy', 'Privacy first') },
              ].map((item, i) => (
                <motion.div 
                  key={i}
        ...
        <section id="demo" className="py-12 sm:py-16 md:py-20 lg:py-28 scroll-mt-20 sm:scroll-mt-24">
          <div className="w-full max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 xl:px-12 2xl:px-16">
            {/* Section Header - Responsive */}
        ...
        {/* Related Features */}
        <RelatedFeatures currentFeature="deviations" />
