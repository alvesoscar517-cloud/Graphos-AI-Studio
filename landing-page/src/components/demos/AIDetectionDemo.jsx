  ...
  return (
    <div className="bg-bg-secondary rounded-2xl border border-gray-200 p-6">
      <h3 className="text-lg font-semibold text-text-primary mb-2">
        {t('demo.tryAIDetection', 'Try AI Detection Demo')}
      </h3>
      <p className="text-sm text-text-secondary mb-4">
        {t('demo.aiDetectionDesc', 'Paste any text below to see a simulated AI detection analysis. This is a demo - for accurate results, use the full application.')}
      </p>
      ...
      <div className="space-y-4">
        <textarea
          ...
          placeholder={t('demo.pasteTextPlaceholder', 'Paste your text here to analyze...')}
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="px-6 py-2 bg-white text-gray-700 border border-gray-300 rounded-lg font-medium hover:bg-gray-50 hover:border-gray-400 transition-colors disabled:opacity-50 disabled:cursor-not-allowed min-w-[100px]"
            >
              {isAnalyzing ? <ThreeDotsLoading size="sm" /> : t('common.analyze', 'Analyze')}
            </button>
          </div>
        </div>
        ...
        {result && (
          ...
          >
            <h4 className="font-semibold text-text-primary mb-4">{t('common.detectionResults', 'Detection Results')}</h4>
            
            {/* Probability Meter */}
            <div className="mb-6">
              <div className="flex justify-between text-sm mb-2">
                <span className="text-success">{t('demo.humanText', 'Human')}: {result.humanProbability}%</span>
                <span className="text-warning">{t('demo.aiText', 'AI')}: {result.aiProbability}%</span>
              </div>
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              <div className="p-3 bg-bg-secondary rounded-lg">
                <div className="text-sm text-text-muted">{t('demo.confidence', 'Confidence')}</div>
                <div className="font-semibold text-text-primary capitalize">{result.confidence}</div>
              </div>
              <div className="p-3 bg-bg-secondary rounded-lg">
                <div className="text-sm text-text-muted">{t('common.wordsAnalyzed', 'Words Analyzed')}</div>
            ...
            <div className="text-center pt-4 border-t border-gray-200">
              <p className="text-sm text-text-secondary mb-3">
                {t('demo.wantMoreAccurate', 'Want more accurate results with detailed analysis?')}
              </p>
              <a
