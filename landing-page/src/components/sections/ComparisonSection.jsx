...
const COMPARISON_FEATURES = [
  {
    ...
    tooltip: 'Detect AI-generated content with industry-leading accuracy',
    graphos: { value: true, note: '98% accuracy' },
    competitors: { value: 'partial', note: 'Basic only' }
  },
  {
    key: 'humanization',
    icon: 'user-check',
    ...
    tooltip: 'Transform AI text to match your unique writing style',
    graphos: { value: true, note: 'Voice-matched' },
  ...
  },
  {
    ...
    tooltip: 'AI learns and replicates your unique writing voice',
    graphos: { value: true, note: 'AI-analyzed' },
  ...
  },
  {
    ...
    tooltip: 'Get instant feedback as you write',
    graphos: { value: true, note: 'Instant' },
    competitors: { value: true, note: 'Delayed' }
  },
  {
    key: 'multiLanguage',
    icon: 'globe',
    ...
    tooltip: 'Write and humanize content in 15+ languages',
    graphos: { value: true, note: '15 languages' },
    competitors: { value: 'partial', note: '2-3 languages' }
  },
  {
    key: 'chromeExtension',
    icon: 'chrome',
    ...
    tooltip: 'Use Graphos AI anywhere on the web',
    graphos: { value: true, note: 'Full features' },
    competitors: { value: 'partial', note: 'Limited' }
  },
  {
    key: 'aiWorkspace',
    icon: 'message-square',
    ...
    tooltip: 'Chat with AI that responds in your writing style',
    graphos: { value: true, note: 'Voice-matched' },
    competitors: { value: false }
  },
  {
    ...
    tooltip: 'Only pay for what you use, no monthly commitment',
    graphos: { value: true, note: 'No subscription' },
    competitors: { value: false, note: 'Monthly only' }
  },
  {
    key: 'creditsExpire',
    icon: 'infinity',
    ...
    tooltip: 'Your credits stay valid forever',
    graphos: { value: true, note: 'Forever yours' },
    competitors: { value: false, note: 'Monthly reset' }
  },
  {
    key: 'freeTier',
    icon: 'gift',
    ...
    tooltip: 'Start free with 100 credits, no credit card required',
    graphos: { value: true, note: '100 credits' },
    competitors: { value: 'partial', note: 'Trial only' }
  }
]
...
const FeatureValue = ({ value, note, isGraphos }) => {
  if (value === true) {
    return (
...
function ComparisonSection() {
  ...
  return (
    <section className="pt-8 lg:pt-12 pb-20 lg:pb-28 relative overflow-hidden">
      ...
      <div className="max-w-5xl mx-auto px-4 relative">
        ...
        >
          ...
          <motion.div
            ...
            whileInView={{ opacity: 1, scale: 1 }}
            viewport={{ once: true }}
            className="inline-flex items-center gap-2 px-5 py-2.5 bg-green-500/10 text-green-600 rounded-full border border-green-500/20"
          >
        ...
        <motion.div
          ...
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          className="bg-bg-primary rounded-2xl border border-gray-200 dark:border-gray-700 overflow-hidden shadow-xl"
        >
          {/* Table Header */}
          <div className="grid grid-cols-3 bg-bg-secondary border-b border-gray-200 dark:border-gray-700">
            <div className="p-4 text-sm font-semibold text-text-secondary flex items-center gap-2">
              <Icon name="list" size="sm" className="text-text-muted" />
              {t('comparison.feature', 'Feature')}
            </div>
            {/* Graphos column with highlight */}
            <div className="p-4 text-center border-l border-gray-200 dark:border-gray-700 bg-gradient-to-b from-primary/10 to-primary/5">
              <div className="flex items-center justify-center gap-2">
                <img src="/logo.svg" alt="Graphos AI" className="w-6 h-6 rounded-lg shadow-sm" />
                <span className="font-bold text-primary text-lg">Graphos AI</span>
              </div>
              <span className="text-xs text-primary/70 mt-0.5 block">Our Solution</span>
            </div>
            <div className="p-4 text-center border-l border-gray-200 dark:border-gray-700">
              ...
              <span className="text-xs text-text-muted mt-0.5 block">Competitors</span>
            </div>
          </div>
            ...
            >
              <div className="py-3 px-4 flex items-center gap-3">
                <Tooltip content={feature.tooltip}>
                  <div className="w-7 h-7 rounded-lg bg-bg-secondary flex items-center justify-center group-hover:bg-primary/10 transition-colors shrink-0">
                    <Icon name={feature.icon} size="sm" className="text-text-muted group-hover:text-primary transition-colors" />
                  </div>
                </Tooltip>
                <Tooltip content={feature.tooltip}>
                  <span className="text-sm font-medium text-text-primary group-hover:text-primary transition-colors cursor-help">
                    {t(feature.labelKey, feature.defaultLabel)}
                  </span>
                </Tooltip>
              </div>
              <div className="py-3 px-4 flex items-center justify-center border-l border-gray-100 dark:border-gray-800 bg-gradient-to-b from-primary/[0.03] to-transparent">
                <FeatureValue 
                  value={feature.graphos.value} 
                  note={feature.graphos.note} 
                  isGraphos={true}
                />
              </div>
              <div className="py-3 px-4 flex items-center justify-center border-l border-gray-100 dark:border-gray-800">
                <FeatureValue 
                  value={feature.competitors.value} 
                  note={feature.competitors.note}
                  isGraphos={false}
                />
              </div>
            </motion.div>
        ...
        >
          <div className="inline-flex items-center gap-6 px-6 py-4 bg-bg-secondary/50 rounded-2xl border border-gray-200 dark:border-gray-700">
            <div className="flex items-center gap-2">
              <div className="flex -space-x-2">
                <img src="https://randomuser.me/api/portraits/women/44.jpg" alt="User" className="w-8 h-8 rounded-full border-2 border-white dark:border-gray-800 object-cover" />
                <img src="https://randomuser.me/api/portraits/men/32.jpg" alt="User" className="w-8 h-8 rounded-full border-2 border-white dark:border-gray-800 object-cover" />
                <img src="https://randomuser.me/api/portraits/women/68.jpg" alt="User" className="w-8 h-8 rounded-full border-2 border-white dark:border-gray-800 object-cover" />
                <img src="https://randomuser.me/api/portraits/men/75.jpg" alt="User" className="w-8 h-8 rounded-full border-2 border-white dark:border-gray-800 object-cover" />
              </div>
              <span className="text-sm text-text-secondary">
                <span className="font-semibold text-text-primary">10,000+</span> writers trust us
              </span>
            </div>
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-sm font-semibold text-text-primary ml-1">4.9/5</span>
            </div>
          </div>
        </motion.div>
        ...
        >
          ...
          <motion.a
            ...
            whileTap={{ scale: 0.98 }}
            className="inline-flex items-center gap-2 px-8 py-4 bg-primary text-white rounded-xl font-semibold text-lg shadow-sm hover:shadow-lg hover:bg-primary-hover transition-all"
          >
